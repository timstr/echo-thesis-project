...
class Echo4ChDatasetRaw(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        assert occupancy_unfolded.size == (64, 4096) # NOTE: PIL uses (width, height) convention
        assert occupancy_unfolded.mode == 'L'

        occupancy_unfolded = self._to_tensor(occupancy_unfolded)
        assert occupancy_unfolded.shape == (1, 4096, 64) # Torch uses (height,width) convention
