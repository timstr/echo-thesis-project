...
def question_marks(height, width, scale, stagger=True):
    ...
    img[(y - hook_center_y)**2 + (x - hook_center_x)**2 <= hook_outer_radius**2] = 0.0
    img[(y - hook_center_y)**2 + (x - hook_center_x)**2 <= hook_inner_radius**2] = 1.0
    ...
    img[
        (y - vertical_stripe_bottom_y)**2 +
        (x - vertical_stripe_x)**2
        <=
        (thickness / 2)**2
    ] = 0.0
