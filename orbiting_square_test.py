...
def render_test_animation(network):
    ds = WaveSimDataset(
        data_folder="dataset/v7_test", permute=False, samples_per_example=1
    )
    ...
    ani = matplotlib.animation.FuncAnimation(
        plt.gcf(), animate, frames=num_frames, interval=1000 / fps
    )
...
if __name__ == "__main__":
    render_test_animation(
        load_model("sdf_from_conv_summary_stats_07-07-2020_12-20-44_model_latest.dat")
    )
