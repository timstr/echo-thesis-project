...
def main():
    ...
    def run_simulation(
        ...
        for b in range(num_batches):
            ...
            res = 3
            rand_init = (
                torch.rand(rad // res, rad // res)
                .repeat_interleave(res, dim=0)
                .repeat_interleave(res, dim=1)
                * 2.0
                - 1.0
            )
            field_now[center - rad : center, center - rad : center] = rand_init.flip(
                [0, 1]
            )
            field_now[center - rad : center, center : center + rad] = rand_init.flip(
                [0]
            )
            field_now[center : center + rad, center - rad : center] = rand_init.flip(
                [1]
            )
            ...
            for i in range(skip_frames + num_steps):
                field_now, field_prev = step_simulation(
                    field_now, field_prev, half_pad_kernel
                )
    ...
    with open("sim_params/current/v7/sim_params_latest.pkl", "rb") as f:
        ...
        half_pad_kernel = torch.tensor(params["half_pad_kernel"], dtype=torch.float).to(
            the_device
        )
