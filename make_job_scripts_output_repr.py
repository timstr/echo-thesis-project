import textwrap
import os
from config import output_format_depthmap, output_format_heatmap, output_format_sdf
...
def make_script(format, implicit, variance):
    desc = f"train_output_repr_9_{format}_{'implicit' if implicit else 'dense'}_{'variance' if variance else 'novariance'}"
    contents = f"""\
    #!/bin/bash
...
for format in [output_format_depthmap, output_format_heatmap, output_format_sdf]:
