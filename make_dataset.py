...
def main():
    ...
    parser.add_argument("--mode", type=str, choices=["normal", "orbiting-circle", "widening-rectangle"], dest="mode", required=True)
    ...
    receiver_indices_chunked = torch.tensor(wavesim_receiver_locations, dtype=torch.long).t().chunk(chunks=num_receivers, dim=0)

    if args.mode == "normal":
        all_configurations = list(all_possible_obstacles(
            max_num_obstacles=2,
            min_dist=0.05,
            min_size=0.1,
            max_size=0.3,
        ...
        all_configurations = [make_circle(t) for t in np.linspace(0.0, 1.0, num_steps, endpoint=False)]
        ...
        all_configurations = [make_rectangles(t) for t in np.linspace(0.0, 1.0, num_steps)]
    else:
    ...
    def make_example(example_index):
        with torch.no_grad():
            ...
            receiver_buf = torch.zeros(num_emitters, num_receivers, wavesim_duration, dtype=torch.float32).to(the_device)
    ...
    if output_path is None or not os.path.exists(output_path):
        raise Exception("Please set the OUTPUT_PATH environment variable to point to the desired output directory")
