    ...
    with torch.no_grad():
        ...
        if input_config.tof_cropping:
            ...
            spectrogram_img_grid = torchvision.utils.make_grid(
                the_input.unsqueeze(1).repeat(1, 3, 1, 1), nrow=1
            )
    ...
    with torch.no_grad():
        if output_config.format == output_format_depthmap:
            ...
            img_grid = torchvision.utils.make_grid(
                gt.unsqueeze(1).repeat(1, 3, 1, 1), nrow=8, pad_value=0.25
            )
...
def show_sampling_location(plt_axis, output_config):
    ...
    thickness = 2.0
    plt_axis.plot(
        [location_x - size, location_x + size],
        [location_y - size, location_y + size],
        c="red",
        linewidth=thickness,
    )
    plt_axis.plot(
