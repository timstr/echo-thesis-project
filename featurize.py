...
def all_yx_locations(size):
    ...
    return (
        torch.stack(torch.meshgrid(ls, ls), dim=2).reshape(size ** 2, 2).permute(1, 0)
    )


def all_zyx_locations(size):
    ls = torch.linspace(0, 1, size).to(the_device)
    zyx = torch.stack(torch.meshgrid((ls, ls, ls)), dim=3)
...
def cutout_circle(barrier, y, x, rad):
    h, w = barrier.shape
    grid = torch.stack(
        torch.meshgrid(torch.linspace(0, 1, h), torch.linspace(0, 1, w)), dim=-1
    )
...
def cutout_square(barrier, y, x, size, angle):
    ...
    c = math.cos(angle)
    s = math.sin(angle)
    rotmat = torch.tensor([[c, s][-s, c]], dtype=torch.float)
...
def sdf_batch_circle(coordinates_yx_batch, circle_y, circle_x, circle_r):
    ...
    circle_yx = (
        torch.tensor([circle_y, circle_x], dtype=torch.float).unsqueeze(-1).to(dev)
    )
    return (
        torch.sqrt(torch.sum((coordinates_yx_batch - circle_yx) ** 2, dim=0)) - circle_r
    )


def sdf_batch_rectangle(
    coordinates_yx_batch, rect_y, rect_x, rect_h, rect_w, rect_angle
):
    assert len(coordinates_yx_batch.shape) == 2
    assert coordinates_yx_batch.shape[0] == 2
    dev = coordinates_yx_batch.device
    ...
    rect_hw_halved = (
        torch.tensor([rect_h * 0.5, rect_w * 0.5], dtype=torch.float)
        .unsqueeze(-1)
        .to(dev)
    )
...
def heatmap_batch(coordinates_yx_batch, obs):
    ...
    for o in obs[1:]:
        vals = torch.max(
            torch.stack(
                (vals, heatmap_batch_one_shape(coordinates_yx_batch, o)), dim=1
            ),
            dim=1,
        )[0]
    return vals
...
def lines_of_sight_from_bottom_up(obs, values):
    ...
    ry_rx_dy_dx = torch.stack(
        (torch.ones(N), values, -torch.ones(N), torch.zeros(N)), dim=0
    )
...
def inigo_quilez_sdf_colours(img):
    ...
    col *= 0.8 + 0.2 * torch.cos(
        60.0 * img
    )  # Note: changed from original 140.0 to make lines less dense
...
def red_white_blue_banded(img):
    ...
    rgb = torch.stack((r, g, b), dim=2) * (0.5 + 0.5 * x.unsqueeze(-1))
    z = torch.clamp(1.0 - 20.0 * torch.abs(img), min=0.0, max=1.0).unsqueeze(-1)
    z_color = (
        torch.tensor([0.0, 0.5, 0.0], dtype=torch.float)
        .unsqueeze(0)
        .unsqueeze(0)
        .to(img.device)
    )
...
def make_heatmap_image_gt(example, img_size):
    ...
    return torch.clamp(img_size * -sdf_batch(coordinates_yx, obs), 0.0, 1.0).reshape(
        img_size, img_size
    )
...
def make_depthmap_pred(example, img_size, network):
    ...
    d = DeviceDict({"input": example["input"][:1], "params": locations})
    pred = network(d)["output"]
...
def make_echo4ch_heatmap_volume_pred(
    example, resolution, network, num_splits, predict_variance
):
    return make_volume_pred(example, resolution, network, num_splits, predict_variance)


def make_echo4ch_depthmap_image_pred(
    example, resolution, network, num_splits, predict_variance
):
    ...
    for size in np.linspace(min_size, max_size, num_size_increments):
        ...
        for y in np.linspace(top + halfsize, bottom - halfsize, num_space_increments):
            for x in np.linspace(
                left + halfsize, right - halfsize, num_space_increments
            ):
                yield CIRCLE, y, x, halfsize

                edge_len = (1.0 / math.sqrt(2.0)) * size
                for angle in np.linspace(
                    0.0, 0.5 * np.pi, num_angle_increments, endpoint=False
                ):
                    yield RECTANGLE, y, x, edge_len, edge_len, angle


def all_possible_obstacles(
    max_num_obstacles,
    min_dist,
    min_size,
    max_size,
...
def make_implicit_params_validation(img_size, dims):
    ...
    if dims == 1:
        ...
        return (
            torch.stack(torch.meshgrid((ls, ls)), dim=2)
            .to(the_device)
            .reshape(1, img_size ** 2, 2)
        )
    elif dims == 3:
        return (
            torch.stack(torch.meshgrid((ls, ls, ls)), dim=2)
            .to(the_device)
            .reshape(1, img_size ** 3, 3)
        )
...
def make_deterministic_validation_batches_implicit(example, output_config):
    ...
    for i in range(num_splits):
        ...
        if output_config.using_echo4ch:
            output_full = (
                example["gt_depthmap"] if (fmt == "depthmap") else example["gt_heatmap"]
            )
            for j in range(B):
